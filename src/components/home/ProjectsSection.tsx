...
const projects = [
  ...
  { title: "CMC Softwar Solution", tag: "SOFTWARE", image: "/exploreimage4.png" },
  { title: "Software Development", tag: "SOFTWARE", image: "/exploreimg5.png" },
];
...
export default function ProjectsSection() {
  ...
  return (
    <section className="relative pt-20 lg:pt-32 bg-white">
      {/* Header Container */}
      <div className="container mx-6 mb-16 flex flex-col md:flex-row justify-between items-end gap-6 relative z-10">
        <div>
        ...
        <button className="mr-4 md:mr-8 bg-primary hover:bg-orange-600 text-white font-bold text-[13px] tracking-wider px-8 py-4 rounded-full transition-colors duration-300 shadow-md shadow-primary/20 uppercase whitespace-nowrap shrink-0">
          VIEW PROJECT DETAIL
        </button>
      </div>
      ...
      <div className="relative">
        ...
        {/* Cards Carousel */}
        <div className="relative z-10 container mx-6">
          ...
          <div className="overflow-hidden w-full py-8 -my-8">
            ...
            >
              {projects.map((project, index) => (
                <div 
                  key={index} 
                  className="w-full sm:w-[calc((100%-40px)/2)] lg:w-[calc((100%-80px)/3)] shrink-0 bg-[#0f172a] rounded-[24px] overflow-hidden flex flex-col group transition-all duration-500 ease-in-out shadow-lg shadow-black/20"
                  style={{ transform: `translateX(calc(${currentIndex * -100}% - ${currentIndex * 24}px))` }}
                >
                  <div className="relative aspect-[1/1.1] w-full overflow-hidden pointer-events-none">
                    <Image 
                      src={project.image} 
                      alt={project.title} 
                      fill 
                  ...
                  <div className="py-6 px-4 flex items-center justify-center text-center min-h-[90px] pointer-events-none">
                    <h3 className="text-white font-bold text-[18px] lg:text-[20px] group-hover:text-primary transition-colors duration-300">
                      {project.title}
                    </h3>
                  </div>
                </div>
