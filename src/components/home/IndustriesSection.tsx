        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6 lg:gap-8 max-w-[1200px] mx-auto">
          {industries.map((ind, index) => (
            <motion.div
              ...
              onClick={() => ind.href && router.push(ind.href)}
              className={`bg-white rounded-[20px] p-6 lg:p-8 flex flex-col items-center justify-center text-center shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-gray-100 hover:border-primary/20 hover:shadow-[0_10px_40px_rgba(0,0,0,0.08)] hover:-translate-y-1 transition-all duration-300 group ${ind.href ? 'cursor-pointer' : 'cursor-default'}`}
            >
