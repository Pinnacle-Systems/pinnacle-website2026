...
export default function WhyChooseUsSection() {
  return (
    <section className="py-20 lg:py-24 bg-[#0b132a] relative overflow-hidden mt-12 lg:mt-0">
      {/* Background Elements */}
      ...
      {/* Decorative Blobs */}
      <div className="absolute top-0 left-0 w-[500px] h-[500px] bg-primary/20 rounded-full blur-[120px] pointer-events-none z-0 transform -translate-x-1/2 -translate-y-1/2"></div>
      <div className="absolute bottom-0 right-0 w-[400px] h-[400px] bg-orange-600/10 rounded-full blur-[100px] pointer-events-none z-0 transform translate-x-1/3 translate-y-1/3"></div>
      ...
      <div className="w-full max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-10 relative z-10">
        ...
        <div className="text-center max-w-4xl mx-auto mb-16 lg:mb-20">
          <motion.div
             ...
             viewport={{ once: true }}
          >
            <div className="inline-flex items-center gap-3 px-5 py-2 rounded-full border border-white/10 mb-6 bg-white/5 backdrop-blur-sm shadow-sm">
              <div className="w-4 h-[3px] bg-primary rounded-full"></div>
              <span className="text-white font-bold text-[13px] tracking-[0.15em] uppercase">Pinnacle Systems Advantage</span>
            </div>
          ...
          {reasons.map((reason, index) => {
            ...
            return (
              ...
              >
                ...
                {/* Icon */}
                <div className="relative w-16 h-16 bg-white/5 rounded-2xl flex items-center justify-center mb-6 text-primary group-hover:scale-110 group-hover:bg-primary group-hover:text-white group-hover:shadow-[0_0_20px_rgba(255,90,0,0.4)] transition-all duration-500 border border-white/5 group-hover:border-transparent">
                  <Icon className="w-8 h-8 stroke-[1.5]" />
                </div>
                
                {/* Content */}
                <h3 className="relative text-[18px] lg:text-[19px] font-bold text-white mb-4 text-center transition-colors duration-300">
                  {reason.title}
                </h3>
                <p className="relative text-gray-400 text-[14.5px] leading-relaxed text-center group-hover:text-gray-300 transition-colors duration-300 flex-grow">
                  {reason.description}
                </p>
              </motion.div>
            );
